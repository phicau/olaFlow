...
Foam::surfaceScalarField Foam::incompressibleInterPhaseTransportModel::mut 
(
    const volScalarField& rho
) const
{
    if (twoPhaseTransport_)
    {
        return
            fvc::interpolate(
                mixture_.rho1()*turbulence1_->nut()
              + mixture_.rho2()*turbulence2_->nut()
              );
    }
    else
    {
        return fvc::interpolate(rho*turbulence_->nut());
    }
}
